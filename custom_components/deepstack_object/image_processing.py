...
class ObjectClassifyEntity(ImageProcessingEntity):
    ...
    def process_image(self, image):
        ...
        if self._scale != DEAULT_SCALE:
            ...
            self._image.thumbnail(newsize, resample=Image.LANCZOS)
            self._image_width, self._image_height = self._image.size
            with io.BytesIO() as output:
                self._image.save(output, format="JPEG")
                image = output.getvalue()
        ...
        targets = len(self._targets_found)
        if targets > 0:
        ...
        self._state = dict(Counter(targets_found))  # e.g. {'car':2, 'person':1}

        if self._save_file_folder:
            if targets > 0 or self._always_save_latest_file:
